...
public class GreedyCompactionStrategy extends AbstractCompactionStrategy
{
    private static final Logger logger = LoggerFactory.getLogger(GreedyCompactionStrategy.class);
    ...
    public GreedyCompactionStrategy(ColumnFamilyStore cfs, Map<String, String> options)
    {
        super(cfs, options);
    ...
    public static List<SSTableReader> mostInterestingBucket(List<List<SSTableReader>> buckets, int minThreshold, int maxThreshold)
    {
    	...
    	List<SSTableReader> smallestBucket = buckets.get(0);
    	if (smallestBucket.size() < minThreshold)
    		return Collections.emptyList();

    	LinkedList<Collection<Integer>> combination = Combination.comb(minThreshold, smallestBucket.size());
    	ListIterator<Collection<Integer>> iterator = combination.listIterator();
    	long maxCount = 0;
    	List<SSTableReader> selectedSet = new ArrayList<SSTableReader>(minThreshold);
    	while (iterator.hasNext())
    	{
    		Collection<Integer> c = iterator.next();
    		List<SSTableReader> currentSet = new ArrayList<SSTableReader>(minThreshold);
    		
    		for (Integer i: c)
        		currentSet.add(smallestBucket.get(i));
    		
    		long setCount = findUnionCardinality(currentSet);
        	if(maxCount < setCount)
        	{
        		maxCount = setCount;
    ...
    static class Combination{
        private static Collection<Integer> bitprint(int u){
        ...
        public static LinkedList<Collection<Integer>> comb(int c, int n){
                LinkedList<Collection<Integer>> s= new LinkedList<Collection<Integer>>();
                for(int u= 0;u < 1 << n;u++)
    ...
    private static long findUnionCardinality(List<SSTableReader> sstables)
    {
    	IFilter union = createMergedBloomFilter(sstables);
    	return cardinality(union);
    }
    
    private static IFilter createMergedBloomFilter(List<SSTableReader> sstables)
    {
    	IFilter unionSetFilter = sstables.get(0).getBloomFilter().merge(sstables.get(1).getBloomFilter());
    	for (int i = 2; i < sstables.size(); i++)
    	{
    		unionSetFilter.mergeInPlace(sstables.get(i).getBloomFilter());
    	}
    	
    	return unionSetFilter;
    }
    
    private static long cardinality(IFilter unionSetFilter)
    {
    	return unionSetFilter.cardinality();
    }
